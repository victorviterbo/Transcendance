...
describe("Contact page", () => {
	...
	it("renders the contact gallery with all team portraits", async () => {
		render(
			<CAuthProvider>
		...
		await waitFor(() => {
			...
			expect(screen.getByAltText("yisho portrait")).toHaveAttribute(
				"src",
				"/imgs/pp/yisho.jpg",
			);
			...
			expect(
				screen.getByAltText("kgauthie portrait").closest("a")?.getAttribute("href"),
			).toBe("mailto:kgauthie@student.42lausanne.ch");
			expect(
				screen.getByAltText("vviterbo portrait").closest("a")?.getAttribute("href"),
			).toBe("mailto:vviterbo@student.42lausanne.ch");
