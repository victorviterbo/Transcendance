import { cloneElement, createContext, useCallback, useContext, useEffect, useState, type Context, type ReactElement, type ReactNode } from "react";
import { getLanguage, getLocalization } from "../../localization/localization";

import type { ILangContent, ILangContext, ILangData, ILocalizationData } from "../../types/localizationTypes";
...
const langContext: Context<ILangContext> = createContext<ILangContext>({
	currentLang: "en",
	ttr: (id: string): string => { return id; },
	ttrf: (id: string, _: Record<string, string>) => {return id;},
	ttrfn: (id: string, _: Record<string, ReactElement>) => { return [<div key={"default"}>{id}</div>]},
	ttrn: (value: number, _?: Intl.NumberFormatOptions) => { return value.toString(); },
	ttrd: (value: string | number | Date, _?: Intl.DateTimeFormatOptions) => { return  value.toString(); },
});
...
function CLanguageProvider({ children }: CLanguageContextProps) {
	...
	//====================== FUNCTIONS ======================
	const ttr = useCallback((id: string): string => {
		if(!langData)
			return id;
	...
	const ttrf = useCallback((id: string, params: Record<string, string>): string => {
		let text = ttr(id);
	...
	const ttrfn = useCallback((id: string, params: Record<string, ReactElement>): ReactNode[] => {
	...
	const ttrn = useCallback((value: number, options?: Intl.NumberFormatOptions): string => {
		const locale = NUMBER_FORMAT_LOCALES[currentLang] ?? NUMBER_FORMAT_LOCALES.en;
		return new Intl.NumberFormat(locale, options).format(value);
	}, [currentLang]);

	const ttrd = useCallback((value: string | number | Date, options?: Intl.DateTimeFormatOptions): string => {
		const locale = NUMBER_FORMAT_LOCALES[currentLang] ?? NUMBER_FORMAT_LOCALES.en;
