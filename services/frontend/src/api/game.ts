...
export function gameCheckErrors<_RES_T extends {error? :IErrorStruct}, Key extends keyof _RES_T>(res: AxiosResponse<_RES_T, unknown, {}>, target: Key) {
	if (!res || !res.data)
...
export async function gameFetchData<_DATA, _RES_T extends {error? :IErrorStruct}, Key extends keyof _RES_T>(
		Request: string,
		target: Key, 
