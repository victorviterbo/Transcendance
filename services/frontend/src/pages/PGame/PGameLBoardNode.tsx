...
import { PGameLBoardNodeStyle, PGameLBoardNodeUsernameStyle, PGameLBoardNodePtsStyle } from "../../styles/pages/game/PGameLBoardNodeStyle"
...
function PGameLBoardNode({user, position}: PGameLBoardNodeProps) {
	return <Stack direction={"row"} sx={PGameLBoardNodeStyle(position)}>
		<CText sx={PGameLBoardNodePtsStyle}>{(position + 1) + "."}</CText>
		<CAvatar src={user.user.image} />
		<CText noTr={true} sx={PGameLBoardNodeUsernameStyle}>{user.user.username}</CText>
