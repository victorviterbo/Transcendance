...
| `userprofile_profile` (`Profile`)   | Public identity for a registered user or anonymous guest. | `id bigint PK`; `uid uuid UNIQUE`; `user_id bigint UNIQUE NULL FK → SiteUser`; `username varchar(20)` with conditional uniqueness; `avatar varchar(100) NULL` (media path); `exp_points integer`; `session_key varchar(40) NULL INDEX`; `guest`, `is_online boolean`; `active_ws_connections integer`; `created_at`, `last_active timestamptz`.   |
